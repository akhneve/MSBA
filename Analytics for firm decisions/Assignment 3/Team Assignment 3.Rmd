...
1. The magnitude of own price elasticity decreases in magnitude significantly to -0.4807.
2. The cross price elasticity of Tide 64 oz on Tide 128 oz decreases down to 0.1242, having a positive effect on the demand. That means when the price of Tide 64 oz increases, the demand of Tide 128 oz increases which makes sense (a Tide customer will choose a product of the same brand).
3. The cross price elasticity of Wisk 64 oz on Tide 128 oz becomes -0.3525, having a negative effect on the demand. That means when the price of Wisk 64 oz increases, the demand of Tide 128 oz decreases.

For the regression results of Tide 64 oz, we have the following:
1. The magnitude of own price elasticity increases in magnitude significantly to -1.2253.
2. The cross price elasticity of Tide 128 oz on Tide 64 oz is -0.7578 having a negative effect on the demand. That means when the price of Tide 128 oz increases, the demand of Tide 64 oz decreases.
3. The cross price elasticity of Wisk 64 oz on Tide 64 oz decreases in magnitude to -0.4476 having a negative effect on the demand. That means when the price of Wisk 64 oz increases, the demand of Tide 64 oz decreases.

Some of the cross-price elasticities for both of the products are negative which doesn't make sense. We have a hypothesis that it is because maybe the prices of all the three products are moving together. That is prices of all three products almost rise and fall together making them interactive with each other rather than independent.

Now, comparing the estimates to the regression with the log of unit sales, not log of velocity, as dependent variable
```{r}
...
Making the functions to calculate the exp volumes of tide 64 oz, tide 128 oz and profits of tide

```{r}
expected_volume_tide_64 <- function(delta_price_tide_128,delta_price_tide_64,model_64,base_volume_tide64){
  volume = model_64$coefficients[["log(p_tide128)"]]*delta_price_tide_128 + model_64$coefficients[["log(p_tide64)"]]*delta_price_tide_64 + base_volume_tide64
  return(volume)
}

expected_volume_tide_128 <- function(delta_price_tide_128,delta_price_tide_64,model_128,base_volume_tide128){
  volume = model_128$coefficients[["log(p_tide128)"]]*delta_price_tide_128 + model_128$coefficients[["log(p_tide64)"]]*delta_price_tide_64 + base_volume_tide128
  return(volume)
}

expected_profit_tide <- function(base_price_tide64,base_price_tide128,delta_price_tide_64,delta_price_tide_128,base_volume_tide64,base_volume_tide128,retail_margin,marginal_cost,model_64,model_128){
  
  profit_tide_64 = expected_volume_tide_64(delta_price_tide_128,delta_price_tide_64,model_64,base_volume_tide64)*(base_price_tide64*(1+delta_price_tide_64)*(1-retail_margin)-marginal_cost)
  
  profit_tide_128 = expected_volume_tide_128(delta_price_tide_128,delta_price_tide_64,model_128,base_volume_tide128)*(base_price_tide128*(1+delta_price_tide_128)*(1-retail_margin)-marginal_cost)
  
  profit_tide = profit_tide_64 + profit_tide_128
  
  return(profit_tide)
}
```

Calculate the total new expected volume of Tide, i.e. the new volume of the 128 oz and 64 oz products, from the following price changes:

A simultaneous 5 percent increase in the prices of Tide 128 and Tide 64
```{r}
delta_price_tide_128 <- 0.05
delta_price_tide_64 <- 0.05

exp_vol_tide64_1 <-expected_volume_tide_64(delta_price_tide_128,delta_price_tide_64,reg_tide64_time_nonpromoted_fixed_2,base_volume_tide64)
exp_vol_tide64_1

exp_vol_tide128_1 <-expected_volume_tide_128(delta_price_tide_128,delta_price_tide_64,reg_tide128_time_nonpromoted_fixed_2,base_volume_tide128)
exp_vol_tide128_1
```
A simultaneous 5 percent decrease in the prices of Tide 128 and Tide 64
```{r}
delta_price_tide_128 <- -0.05
delta_price_tide_64 <- -0.05

exp_vol_tide64_2 <-expected_volume_tide_64(delta_price_tide_128,delta_price_tide_64,reg_tide64_time_nonpromoted_fixed_2,base_volume_tide64)
exp_vol_tide64_2

exp_vol_tide128_2 <-expected_volume_tide_128(delta_price_tide_128,delta_price_tide_64,reg_tide128_time_nonpromoted_fixed_2,base_volume_tide128)
exp_vol_tide128_2
```
A simultaneous 5 percent increase in the price of Tide 128 and 5 percent decrease in the price of Tide 64
```{r}
delta_price_tide_128 <- 0.05
delta_price_tide_64 <- -0.05

exp_vol_tide64_3 <-expected_volume_tide_64(delta_price_tide_128,delta_price_tide_64,reg_tide64_time_nonpromoted_fixed_2,base_volume_tide64)
exp_vol_tide64_3

exp_vol_tide128_3 <-expected_volume_tide_128(delta_price_tide_128,delta_price_tide_64,reg_tide128_time_nonpromoted_fixed_2,base_volume_tide128)
exp_vol_tide128_3
```

A simultaneous 5 percent decrease in the price of Tide 128 and 5 percent increase in the price of Tide 64
```{r}
delta_price_tide_128 <- -0.05
delta_price_tide_64 <- 0.05

exp_vol_tide64_4 <-expected_volume_tide_64(delta_price_tide_128,delta_price_tide_64,reg_tide64_time_nonpromoted_fixed_2,base_volume_tide64)
exp_vol_tide64_4

exp_vol_tide128_4 <-expected_volume_tide_128(delta_price_tide_128,delta_price_tide_64,reg_tide128_time_nonpromoted_fixed_2,base_volume_tide128)
exp_vol_tide128_4
```

Calculate the total new expected profits for each of the price changes in the Q6.f. Are the prices of Tide approximately optimal, or do you recommend changes to the product-line pricing of Tide?
...
expected_profit_1 <- expected_profit_tide(base_price_tide64,base_price_tide128,delta_price_tide_64,delta_price_tide_128,base_volume_tide64,base_volume_tide128,retail_margin,marginal_cost,reg_tide64_time_nonpromoted_fixed_2,reg_tide128_time_nonpromoted_fixed_2)
expected_profit_1
```
...
expected_profit_2 <- expected_profit_tide(base_price_tide64,base_price_tide128,delta_price_tide_64,delta_price_tide_128,base_volume_tide64,base_volume_tide128,retail_margin,marginal_cost,reg_tide64_time_nonpromoted_fixed_2,reg_tide128_time_nonpromoted_fixed_2)
expected_profit_2
```
...
expected_profit_3 <- expected_profit_tide(base_price_tide64,base_price_tide128,delta_price_tide_64,delta_price_tide_128,base_volume_tide64,base_volume_tide128,retail_margin,marginal_cost,reg_tide64_time_nonpromoted_fixed_2,reg_tide128_time_nonpromoted_fixed_2)
expected_profit_3
```
...
expected_profit_4 <- expected_profit_tide(base_price_tide64,base_price_tide128,delta_price_tide_64,delta_price_tide_128,base_volume_tide64,base_volume_tide128,retail_margin,marginal_cost,reg_tide64_time_nonpromoted_fixed_2,reg_tide128_time_nonpromoted_fixed_2)
expected_profit_4
```
...
+===============+===============+=============+==============+===============+
| 0.05          | 0.05          |   282954.2  |    247068    |    2614883    |
+---------------+---------------+-------------+--------------+---------------+
| -0.05         | -0.05         |   282954.4  |    247068    |    2364484    |
+---------------+---------------+-------------+--------------+---------------+
| 0.05          | -0.05         |   282954.3  |    247067.9  |    2521521    |
+---------------+---------------+-------------+--------------+---------------+
| -0.05         | 0.05          |   282954.2  |    247068    |    2457846    |
+---------------+---------------+-------------+--------------+---------------+

A 5% increase in price of both Tide products leads to an increase in profits by roughly 5% which gives the highest increase in profits. So there is a scope of increasing the price of both tide products.

### Question 7: Summarize your findings on the main questions

a. An increase in price of Tide 64 oz leads to increase in sales of Tide 128 oz. This makes sense as now the customers might think if they are paying more, they might as well buy a bigger pack. Although, it is not other way around, which also makes sense as the consumers who want to buy a bigger pack, won't go for a smaller pack and might just prefer to buy another product.

b. For both Tide 64 oz and 128 oz, increase in price of Wisk 64 oz is leading to a lesser sales of our product. Which means that there is a chance our prices are also being increased. Hence, it is difficult to say of we face a competitive threat from Wisk.

c. We believe that currently the pricing is done in reaction to competitors to make our product more apealing. This makes sense as we are able to see our prices go high when Wisk's prices are high. We should maybe change our pricing strategy to be value based pricing instead of competitor based.
